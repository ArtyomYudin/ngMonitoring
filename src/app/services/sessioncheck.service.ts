...
export class SessionCheckService {
  ...
  constructor(private jwtHelper: JwtHelperService) {
    this.isActivate$ = new BehaviorSubject<any>(localStorage.getItem('ngMonitoring') ? true : false);
  ...
  private validate() {
    const validateInterval = setInterval(() => {
      this.isActivate$.next(localStorage.getItem('ngMonitoring') ? !this.jwtHelper.isTokenExpired() : null);
